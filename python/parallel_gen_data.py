...
def _main(
    ...
    pool_args = [(decl_file, dest, rec_limit, depth_limit, weight_limit) for decl_file, dest in zip(gen_data_state.retrieve_tasks(), dests)]
    with ThreadPool(n_workers) as pool:
        finished_count = 0
        for decl_file in tqdm(pool.imap_unordered((lambda x: gen_data_step(*x)), pool_args), total=len(decl_files)):
            finished_count += 1
